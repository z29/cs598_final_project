...
from keras.layers import Input, Dense, SimpleRNN, Masking, Dropout
...
class Network(Model):
    ...
    def __init__(self, dim, batch_norm, dropout, rec_dropout, task,
        ...
        for i in range(depth - 1):
            ...
            rnn_a = SimpleRNN(units=num_units,
                        activation='tanh',
                        return_sequences=True,
                        recurrent_dropout=rec_dropout,
                        dropout=dropout)(X)
                        
            rnn_b = SimpleRNN(units=num_units,
                        activation='tanh',
                        return_sequences=True,
                        recurrent_dropout=rec_dropout,
                        dropout=dropout)(X)
            att_a = attention()(rnn_a)
            att_b = attention()(rnn_b)
            
            out_a = Dense(num_classes, activation=final_activation)(att_a)
            out_b = Dense(num_classes, activation=final_activation)(att_b)
            
           
            
            X = K.sum((out_a*out_b*X*X), axis=1)

        # Output module of the network
        return_sequences = (target_repl or deep_supervision)
        L = SimpleRNN(units=num_units,
                        activation='tanh',
